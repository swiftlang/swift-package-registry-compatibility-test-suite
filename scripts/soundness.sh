...
function replace_acceptable_years() {
    ...
    sed -e 's/2021-2022/YEARS/' -e 's/2021/YEARS/' -e 's/2022/YEARS/'
}
...
if git grep --color=never -i "${unacceptable_terms[@]}" -- . ":(exclude)Tests/PackageRegistryTests/Resources/*.zip" > /dev/null; then
...
tmp=$(mktemp /tmp/.swift-package-registry-compatibility-test-suite_XXXXXX)
