...
struct TestLog: CustomStringConvertible {
    ...
    mutating func start(testCase: String) {
        // Close the current test case before adding another
        self.endCurrentTestCase()
        self.currentTestCase = TestCase(name: testCase)
    }

    mutating func mark(testPoint: String) {
        self.currentTestCase?.mark(testPoint)
    }

    mutating func error(_ message: String) {
        self.currentTestCase?.error(message)
    }

    mutating func error(_ error: Error) {
        self.currentTestCase?.error(error)
    }

    mutating func warning(_ message: String) {
        self.currentTestCase?.warning(message)
    }

    mutating func endCurrentTestCase() {
        if var currentTestCase = self.currentTestCase {
            currentTestCase.end()
            self.testCases.append(currentTestCase)
